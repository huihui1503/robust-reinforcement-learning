...
@dataclasses.dataclass(frozen=True, eq=True, kw_only=True)
#frozen=True - Makes the dataclass immutable. After the object is created, you cannot modify its fields.
#eq=True - 	Instances are compared by value, not by identity.
#kw_only=True - Forces all fields to be set via keyword arguments when constructing the object.
class EvalInEnv:
  ...
  @functools.cached_property #runned once, and then cached.
