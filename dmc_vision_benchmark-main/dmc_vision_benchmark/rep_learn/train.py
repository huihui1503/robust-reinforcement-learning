...
class Trainer:
  ...
  def __init__(self, config: ml_collections.ConfigDict):
    ...
    print("Build online evaluator")
    env = env_wrappers.EvaluationEnvironmentWrapper(
        make_env=env_dmc_vb.make_env,
        make_env_kwargs=dict(
            background_dataset_path=self.config.online_eval.background_dataset_path,#/kubric_movi-d/
            domain_name=self.config.data.domain_name, #walker
            task_name=self.config.data.task_name, #walk
            action_repeats=self.config.online_eval.action_repeats,#2
            frame_stack=self.config.obs_preproc.frame_stack,#3 frames
            height=self.config.data.img_height,# 64
            width=self.config.data.img_width, # 64
            difficulty=self.config.data.difficulty, #None or Medium
            dynamic=self.config.data.dynamic_distractors,#true or false
            background_dataset_videos=self.config.online_eval.background_dataset_videos,#val -> use validation videos in /kubric_movi-d/
    ...
    print("Define online evaluator seeds")
    ...
    self.online_env = eval_in_envs.EvalInEnv(
        ...
        max_steps=self.config.online_eval.max_episode_length, #500
        history_length=self.config.online_eval.history_length, #1
